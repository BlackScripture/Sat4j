...
public class RowSolver {
    ...
    public static void solveRow(PuzzlePiece[] pieces, PuzzleField[] fields, int ersterWert, int currentRow,  List<List<String>> solvedRows, List<List<List<String>>> forbiddenRows, int numberOfCalls) {

        /*
        //Testausgabe zum prüfen ob forbiddenRows korrekt angelegt und gelöscht wird für neue Belegungen der Reihen
        for ( int i = 0; i < forbiddenRows.size(); i++){
            for ( int k = 0; k < forbiddenRows.get(i).size(); k++){
                System.out.println(" ");
                for ( int l = 0; l < forbiddenRows.get(i).get(k).size(); l++){
                    System.out.println(forbiddenRows.get(i).get(k).get(l));
                }
            }
        }
        System.out.println();
